// English dictionary — source of truth for the Dict shape.
// All other languages must conform to this structure.

export const en = {
  nav: {
    posts:   'Posts',
    create:  'Create',
    profile: 'Profile',
  },
  common: {
    back:    'Back',
    save:    'Save',
    cancel:  'Cancel',
    close:   'Close',
    done:    'Done',
    loading: 'Loading…',
    copied:  'Copied to clipboard',
  },
  language: {
    title:   'Language',
    russian: 'Русский',
    english: 'English',
  },
  profile: {
    title:               'Profile',
    currentPlan:         'Current plan',
    managePlan:          'Manage plan',
    channels:            'Channels',
    add:                 '+ Add',
    botSettings:         'Bot settings',
    botSettingsSubtitle: 'Configure Telegram bot',
    language:            'Language',
    appSettings:         'App settings',
    appSettingsSubtitle: 'Notifications, display',
    support:             'Support',
    supportSubtitle:     'Help & feedback',
    connected:           'Connected',
    default:             'Default',
    setDefault:          'Set default',
    channelStyle:        'Channel style',
    monthly:             'Monthly',
    renews:              'Renews',
    active:              'Active',
  },
  plans: {
    title:              'Plans',
    subtitle:           'Choose the plan that fits your channel workflow',
    starter:            'Starter',
    creator:            'Creator',
    studioPro:          'Studio Pro',
    active:             'Active',
    currentPlan:        'Current plan',
    switchToStarter:    'Switch to Starter',
    upgradeToCreator:   'Upgrade to Creator',
    switchToCreator:    'Switch to Creator',
    upgradeToPro:       'Upgrade to Pro',
    month:              '/ month',
    posts30:            '30 AI posts / month',
    posts150:           '150 AI posts / month',
    posts700:           '700 AI posts / month',
    channel1:           '1 channel',
    channels3:          '3 channels',
    channels10:         '10 channels',
    scheduledPosts:     'Scheduled posts',
    postPromotionSoon:  'Post promotion — soon',
    footer:             'All plans include mock data · Payments not implemented',
  },
  posts: {
    tabs: {
      new:       'New',
      scheduled: 'Scheduled',
      published: 'Published',
    },
    status: {
      new:       'New',
      scheduled: 'Scheduled',
      published: 'Published',
    },
    source: {
      bot:           'Bot',
      prompt:        'Prompt',
      link:          'Link',
      text:          'Text',
      forwardedPost: 'Forwarded post',
    },
    meta: {
      variant:  'variant',
      variants: 'variants',
      visual:   'Visual',
    },
    actions: {
      open:          'Open',
      publishNow:    'Publish now',
      cancel:        'Cancel',
      editTime:      'Edit time',
      copy:          'Copy',
      createSimilar: 'Create similar',
    },
    empty: {
      noNew:          'No new posts yet',
      noNewSub:       'Send a message to the bot or use Create',
      noScheduled:    'Nothing scheduled',
      noScheduledSub: 'Schedule posts from the post editor',
      noPublished:    'Nothing published yet',
      noPublishedSub: 'Published posts will appear here',
    },
  },
  create: {
    title:               'Create',
    subtitle:            'Turn a link, idea, or text into a post for your channel.',
    input: {
      link:  'Link',
      idea:  'Idea',
      text:  'Text',
      news:  'News',
      promo: 'Promo',
    },
    placeholder:        'Describe what you want to write about…',
    placeholderLink:    'Paste a URL — article, product page, announcement…',
    placeholderText:    'Paste a text to transform into a post…',
    placeholderNews:    'Paste a news item or announcement…',
    placeholderPromo:   'Describe a promo or offer to create a post for…',
    generateFor:        'Generate for',
    generatePosts:      'Generate posts',
    generating:         'Generating…',
    postsReady:         'Posts ready — opening…',
    generationFailed:   'Generation failed. Try again.',
    howItWorks:         'How it works',
    step1:              'Paste a link, idea, or text',
    step2:              'AI applies your channel style',
    step3:              'Ready variants appear in Posts',
  },
  channelSwitcher: {
    title:       'Switch channel',
    addChannel:  'Add channel',
    connected:   'Connected',
    active:      'Active',
    live:        'Live',
    subscribers: 'subscribers',
  },
  postDetails: {
    source:             'Source',
    variants:           'Variants',
    editText:           'Edit text',
    visual:             'Visual',
    linksButtons:       'Links & buttons',
    publish:            'Publish now',
    schedule:           'Schedule',
    changeSchedule:     'Change schedule',
    openInTelegram:     'Open in Telegram',
    createSimilar:      'Create similar',
    regenerateVariants: 'Regenerate variants',
    save:               'Save',
    copy:               'Copy',
    makeShorter:        'Shorter',
    makeSharper:        'Sharper',
    regenerateText:     'Regenerate text',
    regenerateVisual:   'Regenerate visual',
    template:           'Template',
    postText:           'Post text…',
    chars:              'chars',
    noButtons:          'No link buttons configured.',
    configureLinks:     'Add links in Channel Style settings',
    previewLabel:       'Preview',
    scheduleUpgradeToast: 'Scheduled posts are available on the Creator plan.',
    regenLimitToast:    'Regeneration limit reached for this post. Create a new AI post.',
  },
  schedule: {
    title:          'Schedule post',
    in1h:           'In 1 hour',
    in2h:           'In 2 hours',
    in6h:           'In 6 hours',
    tomorrowMorning: 'Tomorrow 9 AM',
    customTime:     'Custom time',
    schedule:       'Schedule',
  },
  channelStyle: {
    title:    'Channel style',
    notFound: 'No channel style configured for this channel.',
    saved:    'Channel style saved',
    sections: {
      voice:         'Writing style',
      voiceDesc:     'Tone, language, style',
      emoji:         'Emoji',
      emojiDesc:     'Custom emoji rules',
      visual:        'Visual',
      visualDesc:    'Visuals, colors, logo',
      links:         'Links & buttons',
      linksDesc:     'Product & social links',
      signature:     'Signature',
      signatureDesc: 'Post sign-off & CTA',
      rules:         'Post rules',
      rulesDesc:     'Content guidelines',
    },
    save: {
      voice:     'Save writing style',
      emoji:     'Save emoji',
      visual:    'Save visual',
      links:     'Save',
      signature: 'Save signature',
      rules:     'Save rules',
    },
    voice: {
      language:      'Language',
      addressStyle:  'Address style',
      addressTy:     'ты (informal)',
      addressVy:     'вы (formal)',
      tone:          'Tone',
      toneExpert:    'Expert',
      toneCalm:      'Calm',
      toneFounder:   'Founder',
      toneCrypto:    'Crypto',
      toneBold:      'Bold',
      toneMeme:      'Meme',
      postLength:    'Post length',
      short:         'Short',
      medium:        'Medium',
      long:          'Long',
      emojiDensity:  'Emoji density',
      densityNone:   'None',
      densityLight:  'Light',
      densityMedium: 'Medium',
      densityActive: 'Active',
      favoriteWords: 'Favorite words',
      forbiddenWords:'Forbidden words',
      addWord:       'Add word…',
      add:           'Add',
    },
    emoji: {
      packLink:     'Emoji pack link',
      packLinkHint: 'Link a Telegram custom emoji pack',
      strictMode:   'Strict mode',
      strictModeDesc:   'Only use emojis from this pack. No random emoji.',
      fallback:         'Fallback to standard emoji',
      fallbackDesc:     "Use standard emoji if pack emoji aren't available",
      allowed:      'Allowed emoji',
      noEmoji:      'No emoji added yet',
      placeholder:  'Type or paste emoji…',
      add:          'Add',
    },
    visual: {
      brandColor:      'Brand color',
      logo:            'Logo',
      uploadLogo:      'Upload logo',
      logoHint:        'PNG, SVG · appears on visuals',
      backgroundStyle: 'Background style',
      bgDark:          'Dark',
      bgGlass:         'Glass',
      bgGradient:      'Gradient',
      cardStyle:       'Card style',
      cardMinimal:     'Minimal',
      cardBranded:     'Branded',
      cardBold:        'Bold',
      visualTemplate:  'Visual template',
      tplDarkGlass:    'Dark Glass',
      tplMinimal:      'Minimal',
      tplBranded:      'Branded',
      tplNews:         'News',
      watermark:       'Watermark',
      watermarkDesc:   "Add 'CF' watermark to generated visuals",
    },
    links: {
      quickAdd:          'Quick add',
      addLink:           'Add link',
      noUrl:             'No URL',
      editLink:          'Edit link',
      usage:             'Usage',
      usageButton:       'Button under post',
      usageInline:       'Inline link',
      usageSignature:    'Signature',
      usageWhenRelevant: 'When relevant',
      usageAlways:       'Always add',
      label:             'Label',
      url:               'URL',
      anchorText:        'Anchor text',
      buttonLabel:       'Button label',
      save:              'Save',
      cancel:            'Cancel',
    },
    signature: {
      text:        'Signature text',
      cta:         'CTA (optional)',
      usage:       'Usage',
      always:      'Always add',
      whenRelevant:'When relevant',
      never:       'Never',
      preview:     'Preview',
    },
    rules: {
      defaultStructure:    'Default structure',
      defaultStructureHint:'Guide the AI on how to structure posts',
      neverCopySource:     'Never copy source wording',
      neverCopySourceDesc: 'Always create original text from ideas',
      avoidClickbait:      'Avoid clickbait',
      avoidClickbaitDesc:  'No misleading or sensational headlines',
      shortParagraphs:     'Short paragraphs',
      shortParagraphsDesc: 'Maximum 2–3 sentences per paragraph',
      addCta:              'Add CTA if relevant',
      addCtaDesc:          'Include a call-to-action when it fits the post',
      useLinks:            'Use links when relevant',
      useLinksDesc:        'Insert links from the list when contextually relevant',
    },
  },
}

// Dict is inferred without `as const` so values are `string`, not literals.
// This lets other languages satisfy the same shape with different strings.
export type Dict = typeof en
