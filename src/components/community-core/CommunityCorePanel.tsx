...
function PersonaCard({ persona, api, reload }: { persona: Persona; api: (p: string, o?: RequestInit) => Promise<any>; reload: () => Promise<void> }) {
  ...
  return <GlassCard>
    ...
    {open && <div className="mt-4 space-y-4 border-t border-white/[0.06] pt-4">
      ...
      <label className={label}>О себе (био)<textarea rows={2} value={c.identity.about} onChange={e => setId('about', e.target.value)} className={area} placeholder="Короткая суть личности" /></label>
      <label className={label}>Примеры реплик (по одной на строку) — задают голос сильнее всего<textarea rows={4} value={c.voice.messageExamples.join('\n')} onChange={e => setConfig({ ...c, voice: { ...c.voice, messageExamples: e.target.value.split('\n') } })} className={area} placeholder={'да ну, опять эти обещания\nпо факту L2 всё ещё сырые, сам обжигался'} /></label>
      <label className={label}>Канон-факты о себе (по одному на строку) — модель их не выдумывает<textarea rows={3} value={c.canon.join('\n')} onChange={e => setConfig({ ...c, canon: e.target.value.split('\n') })} className={area} placeholder={'8 лет в бэкенде\nпотерял депозит на FTX'} /></label>
      <label className={label}>Интересы (через запятую)<input value={c.interests.join(', ')} onChange={e => setConfig({ ...c, interests: e.target.value.split(',').map(x => x.trim()) })} className={input} /></label>
      <label className={label}>Манера речи<input value={c.voice.speechStyle} onChange={e => setConfig({ ...c, voice: { ...c.voice, speechStyle: e.target.value } })} className={input} /></label>
      <div className="grid grid-cols-2 gap-3">
