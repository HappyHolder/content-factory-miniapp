...
export function ModeratorProbationEditor({ moderatorId }: { moderatorId: string }) {
  ...
  const publish = async () => { if (!initData || publishing) return; setPublishing(true); if (!await save()) { setPublishing(false); return } try { const r = await moderatorFetch(`${API_BASE}/api/moderator-config/${moderatorId}/publish`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ blockType: 'probation' }) }); const data = await r.json() as { error?: string }; if (!r.ok) throw new Error(data.error ?? 'Не удалось применить'); setPublished(true); setMessage('Режим новичка применён') } catch (e) { setMessage(e instanceof Error ? e.message : 'Не удалось применить') } finally { setPublishing(false) } }
