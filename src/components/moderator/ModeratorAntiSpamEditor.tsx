...
const DEFAULT: AntiSpamBlock = { id: 'antispam-default', type: 'antispam', enabled: false, floodEnabled: true, maxMessages: 6, windowSeconds: 10, duplicateEnabled: true, maxDuplicates: 3, duplicateWindowSeconds: 60, linksMode: 'allow', allowedDomains: [], action: 'delete', skipBots: true, skipAdmins: true, skipTrusted: true, raidEnabled: false, raidJoinThreshold: 10, raidWindowSeconds: 60, raidDurationMinutes: 30, blacklistEnabled: false, blacklistThreshold: 2, blacklistAction: 'observe' }
...
export function ModeratorAntiSpamEditor({ moderatorId }: { moderatorId: string }) {
  ...
  const publish = async () => { if (!initData || publishing) return; setPublishing(true); if (!await save()) { setPublishing(false); return } try { const r = await moderatorFetch(`${API_BASE}/api/moderator-config/${moderatorId}/publish`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) }); const data = await r.json() as { error?: string }; if (!r.ok) throw new Error(data.error ?? 'Не удалось применить'); setPublished(true); setMessage('Антиспам применён') } catch (e) { setMessage(e instanceof Error ? e.message : 'Не удалось применить') } finally { setPublishing(false) } }
