...
router.post('/', async (req: Request, res: Response): Promise<void> => {
  ...
  const systemPrompt =
    ...
    `\nPANORAMA PROMPTS: When the user asks for a prompt for Publium panorama generation, first determine the orientation (horizontal swipe carousel or vertical stacked panorama) and the number of segments (2-8). Ask briefly if either is missing. Then return one ready-to-paste image prompt in English, while any explanation stays in the user's language. The prompt must describe ONE seamless continuous scene, never a collage or separate panels. For a horizontal panorama, design a left-to-right visual journey across an ultra-wide canvas; for a vertical panorama, design a top-to-bottom visual journey across an ultra-tall canvas. State the exact segment count, put a meaningful focal detail in every segment, keep faces/hands/key objects away from cut lines, and use backgrounds/light/paths/motion to bridge every seam. Require one camera, perspective, palette, lighting setup and style across the full canvas. Always forbid text, letters, numbers, captions, logos, watermarks, borders, gutters, grids, split screens, storyboards and empty filler. Include the subject, environment, composition, art direction, lighting, palette, depth and camera language instead of vague adjectives. Do not use the word "panorama" as the only composition instruction.\n` +
