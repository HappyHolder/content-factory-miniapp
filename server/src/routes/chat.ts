...
router.post('/', async (req: Request, res: Response): Promise<void> => {
  ...
  const systemPrompt =
    ...
    `\nPANORAMA PROMPTS: When the user asks for a prompt for Publium image slicing, determine one of three existing gallery options: horizontal swipe carousel, vertical stack, or 4x4 modular grid. For horizontal or vertical mode, determine the segment count (2-8) and ask briefly if required information is missing. Return one ready-to-paste image prompt in English, while any explanation stays in the user's language. Horizontal and vertical prompts must describe ONE seamless continuous scene, never a collage or separate panels: use a left-to-right journey for horizontal and a top-to-bottom journey for vertical; state the segment count, put a meaningful focal detail in every segment, keep faces, hands, and key objects away from cut lines, and bridge seams with background, light, paths, or motion. For 4x4 mode, describe ONE normal-looking square 1:1 master image at 4K containing exactly four COMPLETE full-height variants side by side in four invisible equal columns — never sixteen separate parts or scenes. All four variants must share one camera, scale, pose, outer silhouette, ground line, background, and identical connection widths at the 25%, 50%, and 75% horizontal cuts. Allow differences only in surface design, material, color, texture, clothing finish, and small details inside the shared silhouette. For humanoids, explicitly assign band 1 to clean space plus complete head and neck, band 2 to shoulders/chest with symmetrically folded arms fully contained above an identical waist, band 3 to pelvis/upper legs ending at identical knees, and band 4 to lower legs/feet/floor. For other subjects, define four equivalent logical modules with narrow identical connectors. Treat the content generically — people, clothing, products, interfaces, objects, environments, or anything else — and never present the feature as a character constructor. In every mode require the active channel BrandKit art direction, palette, detail, and mood. Always forbid text, letters, numbers, captions, logos, watermarks, visible borders, gutters, guides, labels, and empty filler. Include subject, environment, composition, art direction, lighting, palette, depth, and camera language instead of vague adjectives.\n` +
