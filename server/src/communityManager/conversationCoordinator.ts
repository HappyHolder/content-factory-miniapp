...
export async function recordEpisode(input:{managerId:string;participantId?:string;location:ConversationLocation;kind:string;summary:string;outcome:string}){if(!clean(input.summary))return;await prisma.communityManagerEpisode.create({data:{communityManagerId:input.managerId,participantId:input.participantId,threadId:input.location.threadId,segmentId:input.location.segmentId,kind:input.kind,summary:clean(input.summary,500),outcome:input.outcome}})}
