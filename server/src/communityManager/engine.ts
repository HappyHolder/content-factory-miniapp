...
async function processJob(job:any){
  ...
  const brand=ctx.config.support.useBrandKit?channelAboutContext(ctx.community.channel.brandKit):'',history=conversation.history,previousReplies=await recentCmReplies(ctx.manager.id),ground=k.map((x,i)=>'[K'+(i+1)+' '+x.source+'] '+x.text).join('\n\n'),allowGreeting=allowConversationGreeting(text,Boolean(history||previousReplies));
