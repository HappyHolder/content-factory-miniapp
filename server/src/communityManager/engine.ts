...
async function processJob(job:any){
  ...
  const cooldownFree=await ambientCooldownFree(ctx),route=routeSocialAction({config:ctx.config,decision,telegramDirect,socialAddress,productContext,recentModerator:Boolean(recentModerator&&memory?.pendingModeratorMessageId),cooldownFree,hasQuestion:questionLike(text),unansweredQuestion:openLoop,socialState,personalState});
  const thematic=route.action==='JOIN',moderatorFollowup=route.action==='SUPPORT_MODERATOR';
  const decisionMeta={socialAction:route.action,routeReason:route.reason,engagementLevel:decision.engagementLevel,conversationScore:decision.conversationScore,topic:decision.topic,valueAdd:decision.valueAdd,participantCount:conversation.participantCount,messageCount:conversation.messageCount,thematic,moderatorFollowup,socialAddress,productContext,knowledgeCandidates:k.length,threadCount:conversation.threadCount,socialState,personalState,relationshipState};
  ...
  {
    const checked=await reviewReplyQuality(ctx.config,participantLabel(participant),replyContext,history,text,response,personalState,relationshipState);
