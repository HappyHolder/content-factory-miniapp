...
async function generate(system:string,prompt:string){
  const text=await replicateText({model:env.CM_TEXT_MODEL,systemPrompt:system,prompt,maxTokens:700,timeoutMs:45000,input:{max_completion_tokens:700,reasoning_effort:'low',verbosity:'low'}});return clean(text??'');
}
