...
export async function runDueDailyDigest(managerId:string,now=new Date()){
  ...
  try{
    const raw=await prisma.communityManagerMessage.findMany({where:{communityManagerId:manager.id,createdAt:{gte:window.from,lt:window.to},text:{not:null}},orderBy:{createdAt:'asc'},take:MAX_MESSAGES,select:{telegramMessageId:true,replyToMessageId:true,messageThreadId:true,messageType:true,tgUserId:true,text:true,createdAt:true}}),period=raw.map(item=>({...item,text:item.text??''}));
    ...
    const agent=await runCommunityManagerAgent({managerId:manager.id,communityId:manager.community.id,channelId:manager.community.channelId,channelName:manager.community.channel.name,chatId:manager.community.moderatorChat.tgChatId,config,sessionKey:activitySessionKey('daily_digest',window.dateKey),event:{kind:'DAILY_DIGEST',dedupeKey:'digest-agent:'+manager.id+':'+window.dateKey,activityId:activity.id,digest:{dateKey:window.dateKey,maxTopics:config.activities.dailyDigestMaxTopics,threads}}});
